...
# =============================================================================
st.set_page_config(
    page_title="PRISM — Trial Evidence Reviewer",
    page_icon="📋",
    layout="wide",
)
...
st.markdown(
    ...
    <style>
      ...
      .small-note {font-size: .84rem; color: #5B6675;}
      .sentence-legend {
        padding: .55rem .7rem;
        background: #F8FAFB;
      ...
      .article-model-evidence {
        ...
        padding: .05rem .12rem;
        border-radius: .2rem;
        box-shadow: inset 0 -1px 0 rgba(180, 130, 20, .35);
      }
      .article-human-evidence {
        background: #DCEBFC;
        padding: .05rem .12rem;
        border-radius: .2rem;
        box-shadow: inset 0 -1px 0 rgba(30, 100, 180, .35);
      }
      .article-fp-evidence {
...
GLOSSARY_TERMS: List[Tuple[str, str]] = [
    (
        ...
        "A standard list of facts a clinical trial write-up is expected to report — how "
        "participants were randomized, how many completed the study, what the results were, "
        "and so on. Each fact on the list is called an **item**.",
    ),
    (
        "Item",
        "One specific fact on the reporting checklist, e.g. \"how the sample size was "
        "determined\" or \"who was blinded and how.\"",
    ),
    (
        "Evidence sentence / extraction",
        "A sentence the model found in the paper that answers a given item.",
    ),
    (
        "Reporting status",
        "Whether an item was found in the paper: **Reported** (evidence found), "
        "**Potentially missing** (no evidence found), or **Not evaluated** (not checked yet).",
    ),
    (
        "Compliance check",
        "A study's full checklist scorecard — which items are reported, missing, or "
        "unevaluated, all in one place.",
    ),
    (
        "Correct / Incorrect (review)",
        "A reviewer's verdict on one extracted sentence: does it genuinely support the item "
        "it was matched to?",
    ),
    (
        "True / false positive, false negative",
        "Shorthand from the review step. **True positive**: the model's evidence was correct. "
        "**False positive**: the model's evidence was rejected. **False negative**: evidence "
        "the model missed that a reviewer added by hand.",
    ),
    (
        "Study profile",
        "A normalized one-page summary of a trial — population, intervention, comparator, "
        "outcome, results — built automatically from the accepted evidence.",
    ),
    (
        "Evidence question",
        "The specific comparison a study addresses: which population, which intervention, "
        "against which comparator, measured by which primary outcome.",
    ),
    (
        "Evidence family",
        "A strict grouping of studies that ask the *same* evidence question, so their results "
        "are directly comparable.",
    ),
    (
        "Evidence topic",
        "A broader grouping of evidence families on a related subject — related, but not "
        "necessarily directly comparable (e.g. different doses or comparators).",
    ),
    (
        "Cross-study synthesis",
        "A short generated narrative comparing the studies inside one evidence family.",
    ),
]
...
def render_overview() -> None:
    st.title("Overview")
    st.write(
        "PRISM turns clinical trial write-ups into sentence-level evidence you can check "
        "against a standard reporting checklist, correct by hand, and compare across studies."
    )

    with st.expander("📖 Terms used in this app", expanded=True):
    ...
    with c1:
        with st.container(border=True):
            st.markdown("#### 📥 Bring in a study")
            st.markdown(
                "- Browse studies already processed, or\n"
                "- Upload a new plain-text (`.txt`) trial report\n"
                "- An OpenAI API credential is needed for new uploads"
            )
    with c2:
        with st.container(border=True):
            st.markdown("#### 📤 See what comes out")
            st.markdown(
                "- The full source article\n"
                "- A reporting compliance scorecard\n"
                "- Sentence-level extracted evidence\n"
                "- Your corrections, saved alongside the study\n"
                "- A comparison against related studies"
            )
    with c3:
        with st.container(border=True):
            st.markdown("#### 🧑‍⚕️ Stay in the loop")
            st.markdown(
                "Review every extracted sentence, reject the wrong ones, and add evidence the "
                "model missed — without ever changing the article's underlying sentence IDs."
            )

    st.markdown("## How it works")
    steps = [
        ("1", "Preprocess", "The report is split into ordered paragraphs and sentences with stable identifiers."),
        ("2", "Route", "Each paragraph is assigned to one or more trial-report sections."),
        ("3", "Extract", "Section-specific extractors return item-to-sentence evidence pairs."),
        ("4", "Review", "You see the full paper, reporting status, and extracted evidence together."),
        ("5", "Correct", "Your sentence-level corrections are saved for later model improvement."),
        ("6", "Compare", "Accepted evidence is turned into study profiles and grouped into comparable evidence families."),
    ]
    for number, title, body in steps:
        a, b = st.columns([0.07, 0.93])
        a.markdown(f"<div class='step-badge'>{number}</div>", unsafe_allow_html=True)
        b.markdown(f"**{title}**  \n{body}")

    st.markdown("## Current scope")
    st.markdown(
        "New uploads support plain-text (`.txt`) reports only. Processing covers splitting the "
        "report, routing sections, and extracting item evidence — an additional verification "
        "pass is not run in this prototype."
    )
    ...
    if st.button("Run extraction", type="primary", key="run_live_v6"):
        ...
        if not effective_api_key:
            ...
            try:
                ...
                st.success("Extraction saved. This study is now available under Review studies.")
            except Exception as exc:
                st.exception(exc)
...
with st.sidebar:
    st.markdown("# PRISM")
    st.caption("Extracts and reviews reported evidence from clinical trial write-ups.")
    page = st.radio(
        "Navigation",
        ["Overview", "Review studies", "Upload a report"],
        label_visibility="collapsed",
        key="nav_v6",
    )
    st.divider()
...
if page == "Overview":
    render_overview()
elif page == "Review studies":
    render_saved_results()
elif page == "Upload a report":
    render_upload_new()
